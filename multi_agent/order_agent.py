...
ORDER_SYSTEM_PROMPT = """
    You are ORDER agent specialized to handle all informations about ORDER.

    Order Operations:

        1. order_healthy: check the healthy status ORDER service.       
            - response:
                - content: all information about ORDER service health status and enviroment variables. 
            order_healthy rule:
        ...
        2. get_order: get order details such as sku, name, type, status (IN-STOCK, OUT-OF-STOCK), date of creation (created_at) from a given product(sku)
            - args: 
                - order: id of product.
            - response: 
                - product details such as sku, name, type, status (IN-STOCK, OUT-OF-STOCK), date of creation (created_at)
    
        3. create_order: Create an order (address, currency, user) with the cart item with the list of product.
            - args: 
                - order: order information (address, currency, user) with the cart item with product with a sku, currency, quantity (numeric), price (numeric) 
            - response: 
                - order: An order created with the id and date of creation (created_at).       

        3.checkout_order: Do the checkout payment from a given order.
            - args: 
                - order: the order is and all payments data such as payment currency, amount (numeric value) and type (CASH, CREDIT and DEBIT)
            - response: 
                - checkout: all order information and the payment status.     
...
def order_agent(query: str) -> str:
    ...
    Args:
        query: given an order, create order, checkout order, get order informations and details, and check order healthy status.
    ...
    try:
        ...
        with streamable_http_mcp_server:
            ...
            selected_tools = [
                ...
                if t.tool_name in ["order_healthy", 
                                   "get_order",
                                   "create_order", 
                                   "checkout_order"]
            ]
